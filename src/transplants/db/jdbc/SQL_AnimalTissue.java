...
public class SQL_AnimalTissue {
	...
	public boolean insertAnimalTissue (Animal_tissue animalT){
		try{
			...
			String sql = "INSERT INTO Animal_tissues (name, typeOfTissue, pathology, time) "
					+ "VALUES ('"+ animalT.getName() + "', '" + animalT.getTypeOfTissue() + 
					"' , '" + animalT.getPathology() + "' , '" + animalT.getTimeItLasts() + "');";
	...
	public List <Animal_tissue> searchAnimalTissue (String name){
		...
		try {
			...
			while (rs.next()) {
				...
				Integer timeLasts= rs.getInt("time");
				Animal_tissue animalTiToShow = new Animal_tissue(id, nameAnimalT, 
						                        typeOfTissue, pathology, timeLasts);
	...
	public boolean updateAnimalTissue (Animal_tissue animalTi){
		try {
			...
			prep.setInt(4, animalTi.getTimeItLasts());
